...
class BenchmarkRepository {
  ...
  /// Calculate benchmark stats for rounds within a score range
  BenchmarkStats calculateBenchmarkForScoreRange(
    ...
    final filteredRounds = allRounds.where((round) {
      final diff = (round.totalScore - targetScore).abs();
      return diff <= tolerance;
    }).toList();

    if (filteredRounds.isEmpty) {
      return calculateBenchmark(allRounds);
    }

    return calculateBenchmark(filteredRounds);
  }
  ...
  AggregateStats _calculateAggregateStats(List<Round> rounds) {
    ...
    final fairways = rounds
        .map(
          (r) => r.fairwaysAttempted > 0
              ? r.fairwaysHit / r.fairwaysAttempted * 100
              : 0.0,
        )
        .toList();

    final girs = rounds.map((r) => r.greensInRegulation / 18.0 * 100).toList();
    ...
    final threePuttRate = totalHoles > 0
        ? (threePuttCount / totalHoles) * 100
        : 0.0;
  ...
  Map<String, List<double>> _createDistributions(List<Round> rounds) {
    final List<double> scores = List<double>.from(
      rounds.map((r) => r.totalScore.toDouble()),
    )..sort();

    final List<double> fairways = List<double>.from(
      rounds.map(
        (r) => r.fairwaysAttempted > 0
            ? r.fairwaysHit / r.fairwaysAttempted * 100
            : 0.0,
    ...
    final List<double> girs = List<double>.from(
      rounds.map((r) => r.greensInRegulation / 18.0 * 100),
    )..sort();

    final List<double> putts = List<double>.from(
      rounds.map((r) => r.totalPutts.toDouble()),
    )..sort();

    final List<double> driverDistances = List<double>.from(
      rounds
          .expand((r) => r.shots)
